...
class CycleGAN:
  ...
  def model(self):
    #k_range = [1e-3, 1e-2, 0.1, 1, 10, 100, 1000, 10000]
    #k_range = range(1,11)
    #k_scores = []
    ...
    pre_x = normalize_aittala(x,kernel1)
    #for k in k_range:
        #knn = KNeighborsClassifier(n_neighbors=k)
        #scores = cross_val_score(knn, fake_y, pre_y, cv=10, scoring='accuracy')
        #k_scores.append(scores.mean())
    #print(k_scores)
    
    D_x_loss = self.lambda3*tf.reduce_mean(tf.abs(pre_x - fake_y))
    G_loss = G_gan_loss + cycle_loss + D_x_loss
